package inc.sltechnology.boomplayer.extensions
...
fun Long.waitForCover(context: Context, onDone: (Bitmap?, Boolean) -> Unit) {
    Coil.imageLoader(context).enqueue(
        ImageRequest.Builder(context)
            .data(if (Preferences.getPrefsInstance().isCovers) toAlbumArtURI() else null)
            .target(
...
fun String?.findSorting(launchedBy: String) = Preferences.getPrefsInstance().sortings?.firstOrNull {
    it.albumOrFolder == this && it.launchedBy == launchedBy && it.songVisualization == Preferences.getPrefsInstance().songsVisualization
}

fun Music.findSorting() = Preferences.getPrefsInstance().sortings?.firstOrNull {
    it.albumOrFolder == album && it.launchedBy == launchedBy && it.songVisualization == Preferences.getPrefsInstance().songsVisualization
}

fun Music.findRestoreSorting(launchedBy: String): Int {
    val sorting = when (launchedBy) {
        Constants.ARTIST_VIEW -> findSorting()
        Constants.FOLDER_VIEW -> relativePath?.findSorting(Constants.FOLDER_VIEW)
        else -> album?.findSorting(Constants.ALBUM_VIEW)
    }
...
fun Music.findRestoreSongs(sorting: Int, musicViewModel: MusicViewModel) = when (launchedBy) {
    ...
    Constants.FOLDER_VIEW -> Lists.getSortedMusicListForFolder(
        sorting,
