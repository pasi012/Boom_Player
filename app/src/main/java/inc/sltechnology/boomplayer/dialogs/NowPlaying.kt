...
import inc.sltechnology.boomplayer.databinding.NowPlayingBinding
import inc.sltechnology.boomplayer.databinding.NowPlayingControlsBinding
import inc.sltechnology.boomplayer.databinding.NowPlayingCoverBinding
import inc.sltechnology.boomplayer.databinding.NowPlayingVolControlBinding
...
class NowPlaying: BottomSheetDialogFragment() {
    ...
    private fun setupNPCoverLayout() {
        ...
        mMediaPlayerHolder.let { mph ->
            _npCoverBinding?.run {
                ...
                with (npPauseOnEnd) {
                    isChecked = mPreferences.continueOnEnd
                    setOnCheckedChangeListener { _, isChecked ->
                        mPreferences.continueOnEnd = isChecked
    ...
    private fun setupPreciseVolumeHandler() {
        ...
        _npExtControlsBinding?.run {
            ...
            val isVolumeEnabled = mPreferences.isPreciseVolumeEnabled
    ...
    fun updateNpInfo() {
        if (::mMediaControlInterface.isInitialized) {
            with(mMediaPlayerHolder) {
                (currentSongFM ?: currentSong)?.let { song ->
                    // load album cover
                    if (mAlbumIdNp != song.albumId && mPreferences.isCovers) {
                        loadNpCover(song)
                    }
                    // load album/song info
                    var songTitle = song.title
                    if (mPreferences.songsVisualization == Constants.FN) {
