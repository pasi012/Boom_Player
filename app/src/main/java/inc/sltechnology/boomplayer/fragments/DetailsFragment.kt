...
import inc.sltechnology.boomplayer.databinding.AlbumItemBinding
import inc.sltechnology.boomplayer.databinding.FragmentDetailsBinding
import inc.sltechnology.boomplayer.databinding.GenericItemBinding
import inc.sltechnology.boomplayer.dialogs.Dialogs
import inc.sltechnology.boomplayer.extensions.*
import inc.sltechnology.boomplayer.models.Album
import inc.sltechnology.boomplayer.models.Music
import inc.sltechnology.boomplayer.player.MediaPlayerHolder
import inc.sltechnology.boomplayer.ui.ItemSwipeCallback
...
class DetailsFragment : Fragment(), SearchView.OnQueryTextListener {
    ...
    private val sLaunchedByFolderView get() = mLaunchedBy == Constants.FOLDER_VIEW
    ...
    private val sShowDisplayName get() = Preferences.getPrefsInstance().songsVisualization == Constants.FN
    ...
    private fun setupViews(view: View) {
        ...
        if (sLaunchedByArtistView) {
            ...
            _detailsFragmentBinding?.sortButton?.run {
                ...
                setOnClickListener {
                    ...
                    if (Lists.getUserSorting(mLaunchedBy) == null && Preferences.getPrefsInstance().isSetDefSorting) {
        ...
        view.doOnLayout {
            if (Preferences.getPrefsInstance().isAnimations) {
                _detailsFragmentBinding?.root?.run {
        ...
        _detailsFragmentBinding?.detailsToolbar?.let { tb ->
            ...
            tb.setOnMenuItemClickListener {
                ...
                when (it.itemId) {
                    ...
                    R.id.default_sorting -> applySortingToMusic(Constants.DEFAULT_SORTING)
                    R.id.ascending_sorting -> applySortingToMusic(Constants.ASCENDING_SORTING)
                    R.id.descending_sorting -> applySortingToMusic(Constants.DESCENDING_SORTING)
                    R.id.track_sorting -> applySortingToMusic(Constants.TRACK_SORTING)
                    R.id.track_sorting_inv -> applySortingToMusic(Constants.TRACK_SORTING_INVERTED)
                    R.id.date_added_sorting -> applySortingToMusic(Constants.DATE_ADDED_SORTING)
                    R.id.date_added_sorting_inv -> applySortingToMusic(Constants.DATE_ADDED_SORTING_INV)
                    R.id.artist_sorting -> applySortingToMusic(Constants.ARTIST_SORTING)
                    R.id.artist_sorting_inv-> applySortingToMusic(Constants.ARTIST_SORTING_INV)
    ...
    private fun swapAlbum(songs: MutableList<Music>?) {
        sAlbumSwapped = true
        songs?.get(0)?.findRestoreSorting(Constants.ARTIST_VIEW)?.let { sorting ->
