...
import inc.sltechnology.boomplayer.databinding.FragmentSettingsBinding
...
class SettingsFragment : Fragment() {
    ...
    private fun openLocaleSwitcher() {
        ...
        val dialog: MaterialAlertDialogBuilder = MaterialAlertDialogBuilder(requireContext()).setTitle(R.string.locale_pref_title).setItems(locales.values.toTypedArray()) { _, which ->
            ...
            if (Preferences.getPrefsInstance().locale != newLocale) {
                Preferences.getPrefsInstance().locale = locales.keys.elementAt(which)
                mUIControlInterface.onAppearanceChanged(isThemeChanged = false)
            }
        }.setNegativeButton(R.string.cancel, null)

        if (Preferences.getPrefsInstance().locale != null) {
            dialog.setNeutralButton(R.string.sorting_pref_default) { _, _ ->
                Preferences.getPrefsInstance().locale = null
                mUIControlInterface.onAppearanceChanged(isThemeChanged = false)
            }
        }
        dialog.show()
    ...
    private fun openGitHubPage() {
       val customTabsIntent = CustomTabsIntent.Builder()
        ...
        val parsedUri = getString(R.string.privacy).toUri()
