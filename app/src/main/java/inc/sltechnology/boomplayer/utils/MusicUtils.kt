...
import inc.sltechnology.boomplayer.extensions.toFormattedYear
    ...
    fun updateMediaPlayerHolderLists(uiControlInterface: UIControlInterface, randomMusic: Music?): Music? {
        ...
        fun selectNewSong(filter: Set<String>): Music? {
            if (musicListContains(currentSong, filter)) {
                Preferences.getPrefsInstance().latestPlayedSong = randomMusic
                return randomMusic
            }
            return null
        }
        ...
        Preferences.getPrefsInstance().filters?.let { ft ->
            Preferences.getPrefsInstance().favorites?.toMutableList()?.let { fav ->
                val songs = fav.filter { favFt ->
                    musicListContains(favFt, ft)
                }
                fav.removeAll(songs.toSet())
                Preferences.getPrefsInstance().favorites = fav
                if (fav.isEmpty()) {
